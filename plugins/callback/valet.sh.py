...
#
#  + Ensure the verbose output (-v) is also valetng.
...
class CallbackModule_valetng(CallbackModule_default):

    '''
    This is the valetng callback interface, where screen estate is still valued.
    '''

    CALLBACK_VERSION = 2.0
    CALLBACK_TYPE = 'stdout'
    CALLBACK_NAME = 'valetng'
    ...
    def __init__(self):
        ...
        if HAS_OD:
            ...
            display.warning("The 'valetng' callback plugin requires OrderedDict which is not available in this version of python, disabling.")
            self.disabled = True
...
#        self.keep = True
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        sys.stdout.write('  ' + vt100.green + '[ ok ]' + vt100.reset + ' %s' % name)
    ...
    def v2_playbook_on_task_start(self, task, is_conditional):
        ...
        # Enumerate task if not setup (task names are too long for valetng output)
    ...
    def v2_playbook_on_handler_task_start(self, task):
        ...
        # Enumerate handler if not setup (handler names may be too long for valetng output)
        if task.get_name() != 'setup':
...
if display.verbosity >= 2 or not HAS_OD:
    ...
    CallbackModule = CallbackModule_valetng